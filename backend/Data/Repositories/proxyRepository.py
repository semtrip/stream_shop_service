...
class ProxyRepository:
    ...
    async def get_valid_proxies(self) -> List[Proxy]:
        result = await self.db.execute(
            select(Proxy).filter(Proxy.isValid == True)
        )
        return result.scalars().all()
    ...
    async def add_proxy(self, proxy: Proxy):
        proxy.isValid = False
        proxy.lastChecked = datetime.min
        self.db.add(proxy)
        await self.db.commit()
    ...
    async def get_free_proxy(self) -> Optional[Proxy]:
        result = await self.db.execute(
            select(Proxy)
            .filter(Proxy.isValid == True, Proxy.activeAccountsCount < 3)
