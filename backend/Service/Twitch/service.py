...
class TwtichService():
    def __init__(self, account_manager, proxy_manager, url, count_bots, rump_time, proxies):
    ...
    @classmethod
    async def create(cls, db: AsyncSession, url, count_bots, rump_time=5):
        ...
        return cls(account_manager, proxy_manager, url, count_bots, rump_time, proxies)
    ...
    async def start(self):
        ...
        total_delay_range= self.rump_time.hour * 3600 + self.rump_time.minute * 60 + self.rump_time.second
        for i in range(self.count_bots):
            proxy = self.proxies[i % len(self.proxies)]
            bot = TwitchBot(self.stream_session, self.channel_name, proxy, self.stop_event)
