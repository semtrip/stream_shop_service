...
class LiteViewerBot:
    ...
    async def start(self):
        print(f"[{datetime.now().strftime('%H:%M:%S')}] [BOT-{self.bot_id}] Starting with proxy {self.proxy}")

        # Запускаем aiohttp с прокси для Heartbeat
        ...
        try:
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] [BOT-{self.bot_id}] Connected to stream {stream_quality}")
            await self.connect_to_irc()
            # Запускаем параллельно чтение и heartbeat
    ...
    async def read_stream(self):
        ...
        while True:
            try:
                ...
                if not data:
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] [BOT-{self.bot_id}] Stream ended")
                    break
                print(f"[{datetime.now().strftime('%H:%M:%S')}] [BOT-{self.bot_id}] Received chunk {len(data)} bytes")
    ...
    async def heartbeat_loop(self):
        while True:
            ...
            try:
                async with self.aiohttp_session.post(PING_URL, json=payload) as resp:
                    print(f"[BOT-{self.bot_id}] Heartbeat status: {resp.status}")
            except Exception as e:
                print(f"[BOT-{self.bot_id}] Heartbeat error: {e}")
